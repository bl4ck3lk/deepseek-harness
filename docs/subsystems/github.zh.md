...
[`@deepseek-ai/dsh-github`](../../packages/github/github) 拥有面向 GitHub 合并请求领域的宿主网关。它注册 `ctx.github`，仅通过 `ctx.subprocess` 接缝生成已认证的 `gh` CLI 进程，并发布 `github.*` 一元 Remote 契约。读取经由 `gh api graphql` 执行；评论、评审、合并、关闭与重开经由相应的 `gh pr` 子命令执行。包 README 拥有配置、门控、缓存与错误码语义；本页是这些操作所跨越的公共类型契约。

来源：[`packages/github/github/src/types.ts`](../../packages/github/github/src/types.ts)
