  ...
  for await (const payload of payloads) {
    ...
    for (const choice of chunk.choices ?? []) {
      ...
      for (const call of delta?.tool_calls ?? []) {
        ...
        // OpenAI-compatible gateways vary: continuation deltas may repeat the
        // call id/name as EMPTY STRINGS instead of omitting the field, which
        // would blank the assembled call. Keep the first non-empty value.
